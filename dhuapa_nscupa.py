...
class DHUAPA_NSCUPA(object):
    ...
    def build(self, data_iter):
        ...
        doc_embs, logits = [], []
        with tf.variable_scope('nscupa'):
            nscupa_doc_emb = nsc(input_x, self.max_sen_len, self.max_doc_len,
                                 sen_len, doc_len, [usr, prd],
                                 self.hidden_size, self.emb_dim)
            nscupa_logit = tf.layers.dense(
                nscupa_doc_emb,
                self.cls_cnt,
        ...
        with tf.variable_scope("loss"):
            ...
            lossup = ssce(logits=nscupa_logit, labels=input_y)

            self.loss = self.lambda1 * self.loss + self.lambda2 * lossu + self.lambda3 * lossp + lossup

            sce = tf.nn.softmax_cross_entropy_with_logits_v2
            soft_label = tf.stop_gradient(tf.nn.softmax(nscupa_logit / (self.cls_cnt / 2)))
            align_lossu = sce(labels=soft_label, logits=nscua_logit)
            align_lossp = sce(labels=soft_label, logits=nscpa_logit)
            self.loss += 1. * (align_lossu + align_lossp)
