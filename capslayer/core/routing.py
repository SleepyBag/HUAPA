...
def routing(votes,
    ...
    with tf.variable_scope(name):
        if method == 'EMRouting':
            ...
            pose, activation = dynamicRouting(votes, num_iter, sequence_length, max_length, leaky=False)
...
def dynamicRouting(votes,
    ...
    def _body(i, logits, poses):
        ...
        if sequence_length is not None:
            mask = tf.sequence_mask(sequence_length, max_length)[:, :, None, None, None]
            route = tf.cast(mask, tf.float32) * route
