...
class DHUAPA(object):
    ...
    def dhuapa(self, x, max_sen_len, max_doc_len, sen_len, doc_len, usr, prd,
               convert_flag):
        logits, d_hats = [], []
        for scope, identities in zip(['user_block', 'product_block'],
                                     [[usr], [prd]]):
            with tf.variable_scope(scope):
                logit = self.dnsc(x, max_sen_len, max_doc_len, sen_len,
                                  doc_len, identities, convert_flag)
                logits.append(logit)
    ...
    def dnsc(self, x, max_sen_len, max_doc_len, sen_len, doc_len, identities,
        ...
        with tf.variable_scope('sentence_layer'):
            ...
            for ihop in range(self.sen_hop_cnt):
                ...
                sen_bkg[0] = hop('hop' + str(ihop), last, lstm_outputs,
                                 lstm_bkg, sen_bkg[0], sen_bkg[1:], sen_len,
                                 max_sen_len, convert_flag)
        ...
        with tf.variable_scope('document_layer'):
            ...
            for ihop in range(self.doc_hop_cnt):
                ...
                doc_bkg[0] = hop('hop' + str(ihop), last, lstm_outputs,
                                 lstm_bkg, doc_bkg[0], doc_bkg[1:], doc_len,
                                 max_doc_len, convert_flag)
        outputs = doc_bkg[0]
    ...
    def build(self, data_iter):
        ...
        with tf.variable_scope('inputs'):
            ...
            sen_len = sen_len + 1
            self.max_sen_len += 1
            input_x = tf.reshape(input_x, [-1, self.max_sen_len, self.emb_dim])

        # build the process of model
        d_hat, d_hatu, d_hatp = self.dhuapa(input_x, self.max_sen_len,
                                            self.max_doc_len, sen_len, doc_len,
                                            usr, prd, self.convert_flag)
        prediction = tf.argmax(d_hat, 1, name='prediction')

        with tf.variable_scope("loss"):
            sce = tf.nn.softmax_cross_entropy_with_logits_v2
            self.loss += sce(
                logits=d_hat, labels=tf.one_hot(input_y, self.cls_cnt))
            lossu = sce(
                logits=d_hatu, labels=tf.one_hot(input_y, self.cls_cnt))
            lossp = sce(
                logits=d_hatp, labels=tf.one_hot(input_y, self.cls_cnt))
